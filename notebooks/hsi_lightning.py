...
initial_lr =  0.001 #0.00001 
swa_lr = 0.01
# these should be multiple of 14 for dino model 
img_height = 448
img_width = 448
max_num_epochs = 100
accumulate_grad_batches = 1# 5 # increases the effective batch size  # 1 means no accumulation # more important when batch size is small or not doing multi gpu training
oneformer= False
...
plt.imshow(train_dataset[0][1].permute(1,2,0)/255)
plt.show()
...
trainer = L.Trainer(max_epochs=max_num_epochs, accumulate_grad_batches=accumulate_grad_batches, callbacks=[EarlyStopping(monitor="val_loss", mode="min", verbose=True), checkpoint_callback_val_loss,checkpoint_callback_last_epoch , StochasticWeightAveraging(swa_lrs=swa_lr) ], accelerator="gpu", devices="auto", gradient_clip_val=grad_clip_val, log_every_n_steps=3,  )  # log_every_n_steps=1, gradient_clip_val=grad_clip_val, 
...
# model = UNET_SemanticSegmentation.load_from_checkpoint("lightning_logs/version_17/checkpoints/lowest_val_loss_hsi.ckpt")
model.eval()
